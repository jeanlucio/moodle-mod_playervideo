...
> this class-by-class table does not count. `question_service`/`caption_service` similarly have
> a few branches (the 4.5-only `question_get_default_category()` code path, an already-real
> VTT passthrough) that only a specific Moodle version or input shape reaches. Three domain
> classes with no dedicated test file yet — `local\hud_service`, `local\intro_service`,
> `local\video_source` — are a genuine, acknowledged gap, not a hidden one.
